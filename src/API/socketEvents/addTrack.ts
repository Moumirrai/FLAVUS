...
import { Player, Track, TrackUtils, UnresolvedQuery, UnresolvedTrack } from 'erela.js';
...
const PauseEvent: SocketEvent = {
  ...
  async execute(client, socket, data: IUnresolvedTrack): Promise<any> {
    if (!data || !data.author || !data.duration || !data.title) return socket.emit('playerError', 'Track is corrupted!')
    ...
    if (!player) return socket.emit('playerError', 'Cant add song, there is no player!')
